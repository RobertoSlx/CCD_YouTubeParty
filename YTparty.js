...
function parseData (data) {
  ...
  recomendados.map((actividadRecomendada) =>{    
    ...
    act.categorias = actividadRecomendada.categorias;
    act.recomendada = actividadRecomendada.apareceComoRecomendada;
  ...
  actividadesRec.forEach(calendar => {
      //console.log(calendar);
    esColmena = false;
      endDate = new Date(calendar.fechaFinal);
      if (endDate > nowDate) {
        calendar.categorias.forEach(tag => {
          if (tag.nombre === "La Colmena") { esColmena = true; }
        });
        if(!esColmena){
        actividadesVer.push(calendar);}
    }
  });

  actividadesRec = actividadesVer;
  numActividad = actividadesRec.length - 1;
  console.log(actividadesRec);
...
setInterval(() => {
  posActividad++;
  if(posActividad > numActividad) posActividad = 0; 
